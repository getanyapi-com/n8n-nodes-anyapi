...
test("Get API Schema returns detail schemas with canonical pricing unchanged", async () => {
  ...
  const { output, requests } = await execute(
    { operation: "getSchema", sku: api.slug },
    api,
  );
...
test("Run API follows one accepted POST with database-backed Request polling", async () => {
  const terminal = {
    output: { found: true, data: { email: "hello@example.com" } },
    provider: "AnyAPI",
    costUsd: 0.02,
    items: 1,
  };
  const { output, requests } = await execute(
    ...
    (request) =>
      request.method === "POST"
        ...
        : { requestId: "req_123", status: "succeeded", result: terminal },
  );

  assert.deepEqual(output[0][0].json, terminal);
  assert.equal(
...
test("Run API can return a resumable Request immediately without repeating the POST", async () => {
  const accepted = {
    requestId: "req_123",
    sku: "email_finding.icypeas",
    status: "queued",
...
test("Run API generates a new logical-invocation key when loop iterations reuse an item index", async () => {
  const parameters = {
    operation: "run",
    sku: "email_finding.icypeas",
    inputMode: "json",
...
test("Get API Schema rejects detail without an output schema object", async () => {
  ...
  assert.match(
    output[0][0].json.error,
    /detail\.outputSchema.*expected an object/i,
  );
});
...
test("Browse permits omitted schemas but rejects malformed optional schemas", async () => {
  ...
  assert.match(
    output[0][0].json.error,
    /browse\.apis\[0\]\.inputSchema.*expected an object/i,
  );
});
    ...
    results: [
      {
        ...
        relevance: 4,
        highlightFields: [
          { path: "items[].url", type: "string", why: "Match" },
        ],
      },
    ],
    total: 1,
...
test("Discovery rejects malformed pricing discriminants and negative USD scalars", async () => {
  for (const pricing of [
    {
      from: { model: "tiered", unit: "request", maxUsd: 1 },
      failoverMaxUsd: 1,
    },
...
test("Discovery scans ignored fields and schemas before projection", async () => {
  for (const forbidden of [
    ...
    { future: { provider: "upstream-provider" } },
    {
      inputSchema: {
        type: "object",
        properties: { provider: { provider: "upstream" } },
      },
    },
  ]) {
...
test("Node UI keeps browse category-only and exposes a dedicated search operation", () => {
  ...
  const listFilters = node.description.properties.find(
    ({ name }) => name === "filters",
  );
  const inputMode = node.description.properties.find(
    ({ name }) => name === "inputMode",
  );
  const runOptions = node.description.properties.find(
    ({ name }) => name === "options",
  );

  assert.ok(operations.options.some(({ value }) => value === "search"));
  assert.deepEqual(
    listFilters.options.map(({ name }) => name),
    ["category"],
  );
  assert.ok(inputMode.options.every(({ value }) => typeof value === "string"));
  assert.ok(
    runOptions.options.some(({ name }) => name === "returnImmediately"),
  );
});
